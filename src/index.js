...
class Security {
    ...
    static sanitizeInput(input) {
        ...
        if (typeof input === 'object' && input !== null) {
            return Object.fromEntries(Object.entries(input).map(([key, value]) => [key, Security.sanitizeInput(value)]));
        }
        return input;
    }
...
class PasswordHasher {
    ...
    // Verify password
    static async verifyPassword(password, saltHex, hash) {
...
class JWTManager {
    static async createHmacSigningKey(secret) {
...
// ================= RATE LIMITING =================
class RateLimiter {
    static async checkRateLimit(env, apiKey) {
    ...
    static async recordUsage(env, keyId, userId, endpoint, method, statusCode, responseTime) {
...
class ResponseHelper {
    static jsonResponse(data, status = 200, env) {
    ...
    static errorResponse(message, code = 'GENERIC_ERROR', details, status = 400, env) {
    ...
    static securityHeaders(response) {
...
async function authenticate(request, env) {
    ...
    `).bind(apiKey).first();
        ...
        const user = {
            ...
            email_verified: Number(result.email_verified),
            is_active: Number(result.is_active),
        ...
        const apiKeyData = {
            ...
            is_active: Number(result.key_is_active)
        };
        // Update last used
        await env.DB.prepare(`
      UPDATE api_keys 
...
async function handleSignup(request, env) {
    try {
        ...
        // Generate API key
        const apiKey = 'dsn_' + crypto.getRandomValues(new Uint8Array(16)).reduce((acc, byte) => acc + byte.toString(36), '') + Date.now().toString(36);
...
async function handleDataUpload(request, env, user, apiKey) {
...
async function handleDataSources(request, env, user, apiKey) {
    const startTime = Date.now();
    try {
        const rateLimit = await RateLimiter.checkRateLimit(env, apiKey.key_id);
        if (!rateLimit.allowed) {
