...
async function handleAnalyticsQuery(request: Request, env: Env, user: User, apiKey: ApiKey): Promise<Response> {
  ...
  try {
    ...
    if (analysisType === 'descriptive') {
      ...
      `).bind(sourceId).first<{total_rows: number}>();
...
async function handleReportGenerate(request: Request, env: Env, user: User, apiKey: ApiKey): Promise<Response> {
  ...
  try {
    ...
    `).bind(user.client_id).first<{total_sources: number, total_rows: number}>();
    ...
    `).bind(user.client_id).first<{total_jobs: number, completed_jobs: number}>();
    ...
    const report = {
      ...
      client_name: (await env.DB.prepare('SELECT name FROM clients WHERE id = ?')
        .bind(user.client_id).first<{name: string}>())?.name,
      generated_at: new Date().toISOString(),
...
if (path.startsWith('/api/') && !['/api/signup', '/api/login', '/api/verify-email'].includes(path)) {
  ...
  switch (path) {
    ...
    case '/api/analytics/query':
      if (request.method === 'POST') return handleAnalyticsQuery(request, config, user, apiKey);
      break;
    case '/api/ai/query':
      if (request.method === 'POST') return handleAIQuery(request, config, user, apiKey);
      break;
    case '/api/ai/sentiment':
      if (request.method === 'POST') return handleSentimentAnalysis(request, config, user, apiKey);
      break;
    case '/api/ai/insights':
      if (request.method === 'POST') return handleGenerateInsights(request, config, user, apiKey);
      break;
    case '/api/reports/generate':
      if (request.method === 'POST') return handleReportGenerate(request, config, user, apiKey);
      break;
